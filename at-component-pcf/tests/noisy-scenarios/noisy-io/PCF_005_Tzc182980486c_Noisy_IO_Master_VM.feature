...
#####################################################################################################
@PCF @P2 @Noisy_Scenario @Noisy_IO @Tzc182980486c 

Feature: PCF_005_Tzc182980486c_Noisy_IO_Master_VM

  Scenario: Noisy IO Test On Master VM - Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Noisy IO Test On Master VM - Two Iterations | Fail |
    ...
    # Get Node name
    When I execute the SSH command {config.global.commands.k8s.getmasternodenames} | head -1 at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | NodeName |
...
################## Noisy Scenario Start ######################################
    ...
    Given I loop {SSH.loopCnt} times
    	Given I print: "Running Validation {Constant.index} iteration"
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Noisy IO Test On Master VM - Two Iterations | Pass |
    #CICD report end#####################
