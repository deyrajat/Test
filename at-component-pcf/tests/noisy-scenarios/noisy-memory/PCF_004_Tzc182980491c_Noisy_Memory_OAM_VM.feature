...
#####################################################################################################
@PCF @P2 @Noisy_Scenario @Noisy_Memory @Tzc182980491c 

Feature: PCF_004_Tzc182980491c_Noisy_Memory_OAM_VM

  Scenario: Noisy Memory Test On OAM VM - Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Noisy Memory Test On OAM VM - Two Iterations | Fail |
   ...
   ################## Noisy Scenario Start ######################################
    ...
    When I execute the SSH command "ssh {SSH.NodeName} {config.global.scenario.noisy.mem-required-stress-mem}" at K8SMaster
    Then I save the SSH response in the following variables:
	      | attribute      | value    |
	      | (.*)           | meminfo  |  
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Noisy Memory Test On OAM VM - Two Iterations | Pass |
    #CICD report end#####################