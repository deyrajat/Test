...
####################################################################################################
@PCF @Resiliency @Tzc182637340c @PCF_Resi_Cfg @P3

Feature: PCF_19_3_0_007_Tzc182637340c_Activemq_Pod_Restart

  Scenario: Activemq Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Activemq Pod Restart - One Iterations | Fail |
    ...
    ##Perform Activemq Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never activemq | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Activemq Pod Restart - One Iterations | Pass |
    #CICD report end#####################
