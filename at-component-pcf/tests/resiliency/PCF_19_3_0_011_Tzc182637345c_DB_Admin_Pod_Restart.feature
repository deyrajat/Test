...
####################################################################################################
@PCF @Resiliency @Tzc182637345c @PCF_Resi_CDL @P1 @PcfResP1Set1 @TB4Rerun

Feature: PCF_19_3_0_011_Tzc182637345c_DB_Admin_Pod_Restart

  Scenario: DBAdmin Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | DBAdmin Pod Restart - One Iterations | Fail |
    ...
    ##Perform DBAdmin Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep -E --color=never "db-admin-[0-9]+" | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | DBAdmin Pod Restart - One Iterations | Pass |
    #CICD report end#####################
