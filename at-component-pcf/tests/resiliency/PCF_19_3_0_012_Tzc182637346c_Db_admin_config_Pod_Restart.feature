...
####################################################################################################
@PCF @Resiliency @Tzc182637346c @PCF_Resi_CDL @P2 @PcfResP2Set1 

Feature: PCF_19_3_0_012_Tzc182637346c_Db_admin_config_Pod_Restart

  Scenario: Db-admin-config Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Db-admin-config Pod Restart - One Iterations | Fail |
    ...
    ##Perform Db-admin-config Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^db-admin-config | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Db-admin-config Pod Restart - One Iterations | Pass |
    #CICD report end#####################
