...
####################################################################################################
@PCF @Resiliency @Tzc182637350c @PCF_Resi_Cfg @P2 @PcfResP2Set1

Feature: PCF_19_3_0_014_Tzc182637350c_Network_query_Pod_Restart

  Scenario: Network-query Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Network-query Pod Restart - One Iterations | Fail |
    ...
    ##Perform Network-query Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^network-query | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Network-query Pod Restart - One Iterations | Pass |
    #CICD report end#####################
