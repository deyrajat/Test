...
####################################################################################################
@PCF @Resiliency @Tzc182637352c @PCF_Resi_Cfg @P2 @PcfResP2Set1

Feature: PCF_19_3_0_016_Tzc182637352c_Patch_server_pcf_cnat_cps_infrastructure_Pod_Restart

  Scenario: Patch-server-pcf-cnat-cps-infrastructure Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Patch-server-pcf-cnat-cps-infrastructure Pod Restart - One Iterations | Fail |
    ...
    ##Perform Patch-server-pcf-cnat-cps-infrastructure Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^patch-server-{config.sut.k8s.namespace.pcf}-cnat-cps-infrastructure | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Patch-server-pcf-cnat-cps-infrastructure Pod Restart - One Iterations | Pass |
    #CICD report end#####################
