...
####################################################################################################
@PCF @Resiliency @Tzc182637354c @PCF_Resi_Cfg @P2 @PcfResP2Set1

Feature: PCF_19_3_0_018_Tzc182637354c_Redis_queue_Pod_Restart

  Scenario: Redis-queue Pod Restart 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Redis-queue Pod Restart - Two Iterations | Fail |
    ...
    ##Perform Redis-queue Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^redis-queue | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Redis-queue Pod Restart - Two Iterations | Pass |
    #CICD report end#####################
