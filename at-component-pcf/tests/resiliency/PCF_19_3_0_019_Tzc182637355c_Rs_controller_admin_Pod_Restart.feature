...
####################################################################################################
@PCF @Resiliency @Tzc182637355c @PCF_Resi_Rs @P2 @PcfResP2Set1

Feature: PCF_19_3_0_019_Tzc182637355c_Rs_controller_admin_Pod_Restart


  Scenario: Rs-controller-admin Pod Restart 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs-controller-admin Pod Restart - Two Iterations | Fail |
    ...
    ##Perform Rs-controller-admin Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^rs-controller-admin | grep -v --color=never config | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs-controller-admin Pod Restart - Two Iterations | Pass |
    #CICD report end#####################
