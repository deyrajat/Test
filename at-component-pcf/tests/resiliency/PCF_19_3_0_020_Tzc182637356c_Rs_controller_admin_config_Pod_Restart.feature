...
####################################################################################################
@PCF @Resiliency @Tzc182637356c @PCF_Resi_Rs @P2 @PcfResP2Set1 

Feature: PCF_19_3_0_020_Tzc182637356c_Rs_controller_admin_config_Pod_Restart


  Scenario: Rs-controller-admin-config Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs-controller-admin-config Pod Restart - One Iterations | Fail |
    ...
    ##Perform Rs-controller-admin-config Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^rs-controller-admin-config | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs-controller-admin-config Pod Restart - One Iterations | Pass |
    #CICD report end#####################
