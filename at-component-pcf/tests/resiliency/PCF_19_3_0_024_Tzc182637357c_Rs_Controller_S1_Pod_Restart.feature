...
####################################################################################################
@PCF @Resiliency @Tzc182637357c @PCF_Resi_Rs @P2 @PcfResP2Set2


Feature: PCF_19_3_0_024_Tzc182637357c_Rs_Controller_S1_Pod_Restart

  Scenario: Rs Controller S1 Pod Restart 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs Controller S1 Pod Restart - One Iterations | Fail |
    ...
    ##Perform Rs Controller S1 Pod reboot
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep -E --color=never "rs-controller-spr[0-9]+" | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs Controller S1 Pod Restart - One Iterations | Pass |
    #CICD report end#####################
