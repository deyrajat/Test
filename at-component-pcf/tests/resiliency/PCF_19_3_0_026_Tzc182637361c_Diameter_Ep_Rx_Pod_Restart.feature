...
####################################################################################################
@PCF @Resiliency @Tzc182637361c @PCF_Resi_Engine @P1 @PcfResP1Set1 @TB4Rerun

Feature: PCF_19_3_0_026_Tzc182637361c_Diameter_Ep_Rx_Pod_Restart

  Scenario: Diameter Ep Rx Pod Restart 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Diameter Ep Rx Pod Restart - Two Iterations | Fail |
    ...
    ##Perform Diameter Ep Rx Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never diameter-ep-rx | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never diameter-ep-rx | awk ' FNR == 2 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Diameter Ep Rx Pod Restart - Two Iterations | Pass |
    #CICD report end#####################
