...
####################################################################################################
@PCF @Resiliency @Tzc182634527c @PCF_Resi_Cfg @P2 @PcfResP2Set2 

Feature: PCF_19_3_0_030_Tzc182634527c_Bulk_Stats_Pod_Recreate

  Scenario: Bulk Stats Pod Restart 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Bulk Stats Pod Restart - Two Iterations | Fail |
    ...
    When I execute the SSH command {config.global.commands.k8s.pod-list-cee} | grep --color=never {SSH.PodName} | awk ' FNR == 1 {print $4}' at K8SMaster  
    Then I save the SSH response in the following variables:
      | attribute | value            |
      | (.*)      | PodResstartCount |    
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.cee} | grep --color=never bulk-stats | awk ' FNR == 1 {print $1}' at K8SMaster
	  ...
	  When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.cee}  | grep -v Running at K8SMaster
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Bulk Stats Pod Restart - Two Iterations | Pass |
    #CICD report end#####################
