...
####################################################################################################
@PCF @Resiliency @Tzc182637378c @P2 @NodeNetRestart @Dropped

Feature: PCF_19_3_0_046_Tzc182637378c_Restart_Network_Of_One_Binding_SM_VM

Scenario: Restart Network Of One Binding SM VM 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Restart Network Of One Binding SM VM - Two Iterations | Fail |
    ...
    Given I execute the steps from {config.global.workspace.library.location-features-validations-grafana}Benchmark.feature
 		...
 		##GET name of the Binding SM node.  
    When I execute the SSH command kubectl get nodes  -l smi.cisco.com/node-type=binding-sm -o name | awk -F'/' ' FNR == 1 {print $2}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | NodeName |
    ...
    Then I save the SSH response in the following variables:
      ...
      | (.*)      | grafana_start_time |
		...
		## Now reboot the Binding SM node.  	  
  	When I execute the SSH command ssh -t {SSH.NodeName} "systemctl restart systemd-networkd.service"  at K8SMaster
    	
		Then I wait for {config.global.constant.twohundred.forty} seconds
		Then I wait for {config.global.constant.twohundred.forty} seconds		
			
		## Check the Binding SM node is up.	
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Restart Network Of One Binding SM VM - Two Iterations | Pass |
    #CICD report end#####################
