...
####################################################################################################
@PCF @Resiliency @Tzc182952573c @PCF_Resi_CDL @P2 @PcfResP2Set1


Feature: PCF_19_3_0_066_Tzc182952573c_cps_ldap_ep_Pod_Delete

  Scenario: ldap_ep Pod Delete 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | ldap_ep Pod Delete - Two Iterations | Fail |
    ...
    ##Perform ldap_ep Pod reboot  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep cps-ldap-ep | grep -v svn | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never admin-db | awk ' FNR == 1 {print $1}' at K8SMaster
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | ldap_ep Pod Delete - Two Iterations | Pass |
    #CICD report end#####################
