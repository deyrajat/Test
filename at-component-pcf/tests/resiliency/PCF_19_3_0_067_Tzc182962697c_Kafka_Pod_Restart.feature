###################################################################################################
# Date: <10/10/2019> Version: <Initial version: 19.3.0> Create by <Prosenjit Chatterjee, proschat>
###################################################################################################
@PCF @Resiliency @Tzc182962697c @P1 @PcfResP1Set3

Feature: PCF_19_3_0_067_Tzc182962697c_Kafka_Pod_Restart

  Scenario: Kafka Pod Restart - Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Kafka Pod Restart - Two Iterations | Fail |
    ...
    ##Get name of first Kafka Pod
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never kafka | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Kafka Pod Restart - Two Iterations | Pass |
    #CICD report end#####################
