...
#####################################################################################################
@PCF @Resiliency @Tzc182980450c @P2 @Zookeeper @PcfResP2Set2


Feature: PCF_19_3_0_073_Tzc182980450c_Zookeeper_pod_delete

  Scenario: Zookeeper Pod Delete 30 MINs- Two Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Zookeeper Pod Delete - Two Iterations | Fail |
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^zookeeper | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Zookeeper Pod Delete - Two Iterations | Pass |
    #CICD report end#####################
