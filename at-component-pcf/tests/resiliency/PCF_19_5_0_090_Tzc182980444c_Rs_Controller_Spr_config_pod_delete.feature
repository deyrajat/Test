...
#####################################################################################################
@PCF @Resiliency @Tzc182980444c @P3 @Rs_Controller_Spr 

Feature: PCF_19_5_0_090_Tzc182980444c_Rs_Controller_Spr_config_pod_delete

  Scenario: Rs_Controller_Spr_config Pod Delete 30 MINs- One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs_Controller_Spr_config Pod Delete - One Iterations | Fail |
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never ^rs-controller-spr-config | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never {SSH.PodName} | awk '{print $3}' at K8SMaster
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Rs_Controller_Spr_config Pod Delete - One Iterations | Pass |
    #CICD report end#####################
