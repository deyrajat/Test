...
####################################################################################################
@PCF @Resiliency @Tzc182988194c @P3

Feature: PCF_19_5_0_092_Tzc182988194c_Restart_One_OAM_VM_longer_duration

  Scenario: Restart_One_OAM_VM_longer_duration
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Restart_One_OAM_VM_longer_duration | Fail |
    ...
    When I execute the SSH command kubectl get nodes -l smi.cisco.com/node-type={config.sut.k8s.label.vm.oam} -o name | grep -v {SSH.GrafanaNodename} | awk -F'/' ' FNR == 1 {print $2}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | NodeName |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | Restart_One_OAM_VM_longer_duration | Pass |
    #CICD report end#####################
