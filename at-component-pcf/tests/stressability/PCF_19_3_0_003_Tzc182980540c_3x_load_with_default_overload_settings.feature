...
######################################################################################################
@PCF @Stress @Tzc182980540c @P2 

Feature: PCF_19_3_0_003_Tzc182980540c_3x_load_with_default_overload_settings

  Scenario: 3x_load_with_default_overload_settings - One Iterations
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | 3x_load_with_default_overload_settings - One Iterations | Fail |
    ...
    When I execute the SSH command {config.global.scenario.stress.command-rest-action} at CLIPcfOPSCenter    

    ##Configure the N7 Overload in PCF ops center
    When I execute the SSH command {config.global.scenario.stress.command-rest-overload} {SSH.setThrottleValue} ; commit ; end  at CLIPcfOPSCenter
    Then I receive a SSH response and check the presence of following strings:
      | string               |     occurrence   |
      | Commit complete      |     present      |
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | 3x_load_with_default_overload_settings - One Iterations | Pass |
    #CICD report end##################### 
