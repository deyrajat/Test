#####################################################################################################
# Date: <18/10/2019> Version: <Initial version: 18.0.1> $1Create by <Prosenjit Chatterjee, proschat>
#####################################################################################################
@PCF @Stress @Tzc182891434c @P2

Feature: PCF_19_3_0_004_Tzc182891434c_Duration_Check_for_alert

  Scenario: Duration Check for alert - One Iterations
    ...
    Then I receive a SSH response and check the presence of following strings:
      ...
      | Found logs for Rule : {config.global.scenario.stress.alert-rule-name} with annotation : {config.global.scenario.stress.alert-annotation-summary} | absent     |
