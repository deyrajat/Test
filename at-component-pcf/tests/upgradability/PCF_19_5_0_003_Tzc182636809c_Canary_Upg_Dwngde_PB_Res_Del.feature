...
###########################################################################################
@PCF @PCF_Canary @Tzc182636809c

Feature: PCF_19_5_0_003_Tzc182636809c_Canary_Upg_Dwngde_PB_Res_Del

 Scenario: PCF Canary_Upg_Dwngde_PB_Res_Del 
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | PCF Canary_Upg_Dwngde_PB_Res_Del | Fail |
    ...
    When I execute the SSH command config ; {config.global.scenario.engine.rule-name-supi} ; commit ; end at CLIPcfOPSCenter
    ...
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never policy-builder-{config.sut.k8s.namespace.pcf}-pcf-engine-app | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |
...
#    Given I execute the steps from {config.global.scenario.resiliency.lib-dir}RestartPod.feature
    ...
    ### Workaround due to CDET 
    When I execute the SSH command kubectl exec -it {SSH.PodName} -n {config.sut.k8s.namespace.pcf} -c policy-builder /sbin/killall5 at K8SMaster  
    ...
    When I execute the SSH command config ; {config.global.scenario.engine.rule-name-default-supi} ; commit ; end at CLIPcfOPSCenter
    ...
    ##Perform Policy engine Pod delete  
    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep --color=never policy-builder-{config.sut.k8s.namespace.pcf}-pcf-engine-app | awk ' FNR == 1 {print $1}' at K8SMaster
    Then I save the SSH response in the following variables:
      | attribute      | value    |
      | (.*)           | PodName  |  
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | PCF Canary_Upg_Dwngde_PB_Res_Del | Pass |
    #CICD report end#####################