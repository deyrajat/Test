...
###########################################################################################
@PCF @PCF_Canary @Tzc182636810c

Feature: PCF_19_5_0_004_Tzc182636810c_Canary_Upg_Dwngde_RestEp_Res_Del

 Scenario: PCF Canary_Upg_Dwngde_RestEp_Res_Del 
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | PCF Canary_Upg_Dwngde_RestEp_Res_Del | Fail |
...
#    Given I execute the steps from {config.global.scenario.resiliency.lib-dir}RestartPod.feature
    ...
    #When I execute the SSH command kubectl exec -it {SSH.PodName} -n {config.sut.k8s.namespace.pcf} -c pcf-rest-ep /sbin/killall5 at K8SMaster  
    When I execute the SSH command kubectl delete pod {SSH.PodName} -n {config.sut.k8s.namespace.pcf} --force at K8SMaster  
...
#    When I execute the SSH command kubectl get pod -n {config.sut.k8s.namespace.pcf} | grep -v Running at K8SMaster
    ...
    Then I update report for table {Constant.testtype} with the following details:
      | PCF Canary_Upg_Dwngde_RestEp_Res_Del | Pass |
    #CICD report end#####################