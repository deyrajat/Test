...
keyStoneFile="/root/openstack-configs/corerc"
keyStoneFileBlade="/root/openstack-configs/openrc"
...
function change_vm_state
{
    ...
    VM_DEP_ID=$RET_VAL
    log_info "Shutting down / Start up / Reboot VM $VM_NAME $RET_VAL on ESC $ESC_IP_Address"
    _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD get esc_datamodel | grep -a2 $VM_DEP_ID  | grep generated_name"
    genVMName=$RET_VAL
    genVMName=`echo $genVMName | cut -d'>' -f 2 | cut -d'<' -f 1 `
    _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD vm-action $NEW_VM_STATE $genVMName"
    sleep 120
}
...
function _get_vm_blade_name
{
    ...
    echo "in _get_vm_blade_name BLADE_HOSTNAME : " $BLADE_HOSTNAME
}

function get_blade_ip_for_vm
{
    nodes=$1
    _ssh_command_ret $OSPD_IP_Address $PASSWORD "grep $nodes /root/openstack-configs/mercury_servers_info"
    BladeIP=`echo $RET_VAL | awk '{print $6}'`
    echo "BladeIP : " $BladeIP
}
...
function blade_vm_operation
{
    ...
    do
        echo "the VM to operate is : " $VM_Full_Name
        RET_VAL=""
    	_ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD get esc_datamodel | grep -a2 $VM_Full_Name | grep generated_name"
    	genVMName=$RET_VAL
    	echo "genVMName is :" $genVMName
    	genVMName=`echo $genVMName | cut -d'>' -f 2 | cut -d'<' -f 1 `
    	echo "genVMName is :" $genVMName
   		_ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD vm-action $OPERATION $genVMName"
...
function get_cimc_blade_ip_for_vm
{
    nodes=$1     
    _ssh_command_ret $OSPD_IP_Address $PASSWORD "grep $nodes /root/openstack-configs/mercury_servers_info"
    CIMCBladeIP=`echo $RET_VAL | awk '{print $4}'`
    echo "CIMCBladeIP :" $CIMCBladeIP
...
function change_blade_state
{
    ...
    echo $BLADE_HOSTNAME
    echo "==================="
    get_cimc_blade_ip_for_vm $BLADE_HOSTNAME
    echo $CIMCBladeIP
    #_ssh_command_ret $OSPD_IP_Address $PASSWORD "sshpass -p $PASSWORD ssh $CIMC_USER@$CIMCBladeIP  \"scope chassis ; power $BLADE_OPERATION ; echo \r \""
}   
...
function get_appltcation_blade_sm
{
    ...
    do  
        ...
        _ssh_command_ret $OSPD_IP_Address $PASSWORD "source $keyStoneFile ; nova list --host $BLADE_HOSTNAME | grep -i internal | cut -d '|' -f 7 | cut -d'=' -f 2 | cut -d';' -f 1"
        VM_IP_LIST=$RET_VAL
        takehost=0
        qnscnt=0
...
function restart_blade_network
{
    ...
    echo $BLADE_HOSTNAME
    echo "==================="
    get_blade_ip_for_vm $BLADE_HOSTNAME
    echo $BladeIP
    _ssh_command_ret $OSPD_IP_Address $PASSWORD "ssh $BladeIP service network restart ; echo \r"
}
...
function check_network_on_blade
{
    ...
    get_blade_ip_for_vm $BLADE_HOSTNAME
    echo $BladeIP
    _ssh_command_ret $OSPD_IP_Address $PASSWORD "ssh $BladeIP sudo systemctl status network.service"
    if [[ $RET_VAL =~ .*active.* ]] ; then 
