...
ESC_Source_FilePath="/opt/cisco/esc/confd/confdrc"
keyStoneFile="/home/stack/tb4-ultramrc-core"
...
function change_vm_state
{
    ...
    if [ "$NEW_VM_STATE" == "STOP" ]; then
       VM_IP=`ssh $VM_NAME ifconfig ens3 | grep netmask | awk '{print $2}'`
    ...
    Vm_Search_Name=`echo $VM_NAME | awk -F'-' '{print $2}' | grep --color=never -o '[a-zA-Z]\+'`
    Vm_Search_Digit=`echo $VM_NAME | awk -F'-' '{print $2}' | grep --color=never -o -o '[0-9]\+' | sed 's/^0*//'`
    Search_Vm_Name=`echo $Vm_Search_Name$Vm_Search_Digit`
    _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD get esc_datamodel/opdata | grep $Search_Vm_Name -A 10 | grep generated_name | awk -F'>' '{print \$2}' | awk -F'<' '{print \$1}'"
    VM_Generated_Name=$RET_VAL
    log_info "Shutting down / Start up / Reboot VM $VM_NAME $RET_VAL on ESC $ESC_IP_Address"
    if [ "$NEW_VM_STATE" == "STOP" ]; then
       _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD vm-action DISABLE_MONITOR $VM_Generated_Name"
       echo "disable monitor"
    else
       _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD vm-action ENABLE_MONITOR $VM_Generated_Name"
       echo "enable monitor"
    fi
    sleep 60
    _ssh_esc_command_ret $ESC_IP_Address $ESC_password "$ESC_CLI_CMD vm-action $NEW_VM_STATE $VM_Generated_Name"
}
...
function _network
{
    ...
    do      
        ...
        if [[ $RET_VAL = *"RUNNING"* ]];
        then 
                    log_info "State before: VM: $node  IP: $VM_IP Interfaces: UP"
        else
            log_info "State before: VM: $node  IP: $VM_IP Interfaces: DOWN"
        fi
        RET_VAL=""  
            ...
            if [[ $RET_VAL =~ "RUNNING" ]];
            then 
                    log_info "State after: VM: $node  IP: $VM_IP Interfaces: UP"
        else
            log_info "State after: VM: $node  IP: $VM_IP Interfaces: DOWN"
        fi      
    done
}
...
function network_service_restart
{
    ...
    then 
        log_info "Network restart on $node SUCCESS"
    else
        log_info "Network service restart on $node FAIL"
    fi
}
